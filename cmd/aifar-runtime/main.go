...
func writeMetrics(w http.ResponseWriter, manager *runtimeagent.Manager, build buildInfo) {
	...
	fmt.Fprintf(w, "# HELP aifar_runtime_degraded_runtimes Number of Runtime resources in Degraded phase.\n")
	fmt.Fprintf(w, "# TYPE aifar_runtime_degraded_runtimes gauge\n")
	fmt.Fprintf(w, "aifar_runtime_degraded_runtimes %d\n", metrics.DegradedRuntimeCount)
	fmt.Fprintf(w, "# HELP aifar_runtime_container_restarts Managed container restart attempts recorded by the runtime.\n")
	fmt.Fprintf(w, "# TYPE aifar_runtime_container_restarts counter\n")
	fmt.Fprintf(w, "aifar_runtime_container_restarts %d\n", metrics.ContainerRestarts)
